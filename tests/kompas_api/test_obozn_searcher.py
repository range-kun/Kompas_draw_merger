from unittest.mock import create_autospec, Mock, patch, call

import pytest

from programm.kompas_api import OboznSearcher, KompasAPI, SpecType, NotSupportedSpecType, NoExecutions
from programm.schemas import FilePath, SpecSectionData, DrawData
...
class TestOboznSearcher:
    ...
    def preset_data(self):
        ...
        return {"spc_description": [1, 2, 3], "get_line_section": [5, 15, 20], "get_line_obozn_name_size": lines_data}
    ...
    def get_all_lines_with_correct_type(self):
        with patch.object(OboznSearcher, "_get_all_lines_with_correct_type", return_value=[Mock()]):
            yield

    @pytest.fixture
    def regular_oformlenie(self, kompas_api, preset_data, get_all_lines_with_correct_type):
        kompas_api.create_spc_object.return_value = SpecType.REGULAR_SPEC, preset_data["spc_description"]

    @pytest.fixture
    def group_oformlenie(self, kompas_api, preset_data, get_all_lines_with_correct_type):
        kompas_api.create_spc_object.return_value = SpecType.GROUP_SPEC, preset_data["spc_description"]
    ...
    @pytest.fixture
    def obozn_searcher_regular(self, spec_path, kompas_api, regular_oformlenie, get_line_draws_data):
        return OboznSearcher(spec_path, kompas_api)

    @pytest.fixture
    def obozn_searcher_group(self, spec_path, kompas_api, group_oformlenie, get_line_draws_data):
        return OboznSearcher(spec_path, kompas_api)

    @pytest.fixture
    def mock_verify_column_not_empty(self):
        with patch.object(OboznSearcher, "_verify_column_not_empty") as mock_verify_column:
    ...
    def get_empty_line_obozn_name_size(self):
        with patch.object(OboznSearcher, "_get_line_obozn_name_size") as lines_data:
            lines_data.side_effect = [(None, None, None), ]
    ...
    def test_correct_returned_executions(self, obozn_searcher_group, mock_verify_column_not_empty):
        with patch.object(OboznSearcher, "_get_cell_data") as mock_cell_data:
            ...
            assert executions == {"draw_1": 1, "Базовое исполнение": 2, "Все исполнения": 1000}

    def test_no_executions_with_exception_raise(self, obozn_searcher_group):
        with patch.object(OboznSearcher, "_get_cell_data", side_effect=[AttributeError]):
    ...
    def test_should_user_correct_function_by_oformlenie(self, obozn_searcher_regular, kompas_api):
        with patch("programm.kompas_api.OboznSearcher._get_obozn_from_simple_specification",
    ...
    def test_get_executions_should_use_provided_column_number(self, obozn_searcher_group):
        with patch.object(OboznSearcher, "_get_obozn_from_group_spec", return_value=[Mock(), Mock()]):
    ...
    def test_simple_spec_to_get_only_assemblies(self, spec_path, kompas_api, regular_oformlenie, get_line_draws_data):
        ...
        assert spec_data == (
            [
                SpecSectionData(draw_type=DT.ASSEMBLY_DRAW,
    ...
    def test_simple_spec_to_get_all_type_of_draws(self, obozn_searcher_regular):
        ...
        assert spec_data == (
            [
             SpecSectionData(draw_type=DT.ASSEMBLY_DRAW,
                             draw_names=[DrawData(draw_obozn="ABC-00.00.00 SB", draw_name="Assembly Draw")]),
             SpecSectionData(draw_type=DT.DETAIL,
                             draw_names=[DrawData(draw_obozn="ABC-10.00.01", draw_name="Detail")]),
             SpecSectionData(draw_type=DT.SPEC_DRAW,
    ...
    def test_simple_spec_to_call_internal_methods(self, obozn_searcher_regular):
        with patch.object(OboznSearcher, "_create_spec_output") as mocked_output:
            with patch.object(OboznSearcher, "_parse_lines_for_detail_and_spec") as mocked_parser:
    ...
    def test_get_line_obozn_name_size(self, regular_oformlenie, kompas_api, spec_path, preset_data):
        with patch.object(OboznSearcher, "_get_cell_data") as mock_cell_data:
            with patch.object(OboznSearcher, "_parse_lines_for_detail_and_spec") as mocked_parser:
                # given: regular spec to get all lines
                mock_cell_data.side_effect = (
                    "ABC-00.00.00 SB", "Assembly Draw", "A1",
                    "ABC-00.00.00 XX", "Speca Draw", "A2",
                    "ABC-10.00.01", "Detail", "A4"
                )
    ...
    def test_get_line_obozn_name_size_procced_exception(self, regular_oformlenie, kompas_api, spec_path, preset_data):
        with patch.object(OboznSearcher, "_get_cell_data") as mock_cell_data:
            with patch.object(OboznSearcher, "_parse_lines_for_detail_and_spec") as mocked_parser:
                # given: regular spec to get all lines with one not existing cell
                mock_cell_data.side_effect = (
                    "ABC-00.00.00 SB", "Assembly Draw", "A1",
                    "ABC-10.00.01", "Detail", AttributeError,
                    "ABC-00.00.00 XX", "Speca Draw", "A2",
                )
                searcher = OboznSearcher(spec_path, kompas_api)
    ...
    def test_should_check_if_detail_passed_as_spec(self, obozn_searcher_regular, kompas_api):
        ...
        assert spec_data == (
            ...
            [f"\nВозможно указана деталь в качестве спецификации -> {self.SPEC_PATH} ||| ABC-10.00.01 \n"]
        )

    @pytest.mark.parametrize(
        "obozn_data,result",
        [("ABC-00.00.00", False), ("ABC-00.00.00А", False), ("ABC-00.00.01", True), ("ABC-00.00.01А", True)])
    def test_is_detail_method_correct_work(self, obozn_searcher_regular, obozn_data, result):
    ...
    @pytest.mark.parametrize("spec_obozn,result", [("ABC-00.00.00-01", "01"), ("ABC-00.00.00А", "-")])
    def test_get_obozn_execution_if_provided(
        ...
        with patch.object(OboznSearcher, "_get_column_number_by_execution") as mocked_method:
    ...
    @pytest.mark.parametrize("spec_obozn,execution,return_result",
    ...
    def test_get_column_number_by_execution(
        ...
        with patch.object(OboznSearcher, "_get_cell_data") as mock_cell_data:
            ...
            # +1 потому что при поиске в методе _get_column_number_by_execution начинаем итерироваться от 1
            mock_group_searcher_result.assert_called_with([return_result.index(execution) + 1])
            mock_verify_column_not_empty.assert_called_with(return_result.index(execution) + 1)
    ...
    def test_exception_if_not_provided_correct_value_to_get_column_num(
        ...
        with patch.object(OboznSearcher, "_get_cell_data") as mock_cell_data:
            ...
            obozn_searcher = OboznSearcher(spec_path, kompas_api, spec_obozn=f"ABC-00.00.00")
    ...
    def test_group_spec_to_get_all_type_of_draws(
        ...
        with patch.object(OboznSearcher, "_get_cell_data", return_value=True) as mocked_cell_data:
            with patch.object(OboznSearcher, "_get_all_lines_with_correct_type", return_value=[Mock()] * 3):
    ...
    def test_group_spec_to_get_only_unique_rows(
        ...
        with patch.object(OboznSearcher, "_get_cell_data", return_value=True) as mocked_cell_data:
            with patch.object(OboznSearcher, "_get_all_lines_with_correct_type", return_value=[Mock()] * 5):
                with patch.object(OboznSearcher, "_get_line_obozn_name_size") as lines_data:
                    lines_data.side_effect = [
                        ("ABC-00.00.00 SB", "Assembly Draw", "A1"),
                        ("ABC-10.00.00", "Speca", "A2"),
    ...
    def test_group_searcher_call_internal_methods(self, obozn_searcher_group, patch_get_column_numbers):
        with patch.object(OboznSearcher, "_parse_lines_for_detail_and_spec", return_value=True) as mocked_parser:
            with patch.object(OboznSearcher, "_create_spec_output") as mocked_output:
                # when: called method to get spec data
                obozn_searcher_group.get_obozn_from_specification()
